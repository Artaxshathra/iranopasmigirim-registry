...
describe('player.html', () => {
  ...
  it('scripts reference only local files (no remote origins — MV3 forbids them)', () => {
    const scripts = attrs(html, 'script');
    for (const s of scripts) {
      const src = attrValue(s, 'src');
      if (src) {
        assert.ok(!src.startsWith('http'),
          `script src must be local — Chrome MV3 rejects remote script origins: ${src}`);
      }
    }
  });
  ...
  it('no #btn-cast button — cast UI was removed in 1.2.2', () => {
    // See CHANGELOG 1.2.2: Chrome users get casting via right-click → Cast.
    const castBtn = attrs(html, 'button').find(b => attrValue(b, 'id') === 'btn-cast');
    assert.ok(!castBtn, '#btn-cast must not exist');
  });

  it('video element has x-webkit-airplay="allow" for Safari native AirPlay', () => {
    // No SDK or extra UI needed — Safari surfaces its own AirPlay button
    // inside the native video controls bezel when this attribute is set.
    const [videoTag] = attrs(html, 'video');
    assert.match(videoTag, /x-webkit-airplay=["']allow["']/,
      'video must have x-webkit-airplay="allow"');
  });
